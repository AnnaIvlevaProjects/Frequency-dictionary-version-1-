...
def test_add_xml_paths_handles_duplicate_root_segment(tmp_path: Path):
    # corpus_root ends with post1950, while metadata path also starts with post1950/
    root = tmp_path / "texts" / "post1950"
    target = root / "child" / "fiction" / "asmolov"
    target.mkdir(parents=True)
    (target / "belosh01.xml").write_text("<doc/>", encoding="utf-8")

    rows = [{"path": "post1950/child/fiction/asmolov/belosh01"}]
    enriched = add_xml_paths(rows, root)

    assert enriched[0]["xml_exists"] is True
    assert str(enriched[0]["xml_abs_path"]).endswith("belosh01.xml")


def test_add_xml_paths_handles_sibling_corpus_root(tmp_path: Path):
    # corpus_root points to post1950, but metadata path points to pre1950/...
    root = tmp_path / "texts" / "post1950"
    sibling_target = tmp_path / "texts" / "pre1950" / "xix" / "bulgarin"
    sibling_target.mkdir(parents=True)
    (sibling_target / "bulg-01.xml").write_text("<doc/>", encoding="utf-8")

    rows = [{"path": "pre1950/xix/bulgarin/bulg-01"}]
    enriched = add_xml_paths(rows, root)

    assert enriched[0]["xml_exists"] is True
    assert str(enriched[0]["xml_abs_path"]).endswith("pre1950/xix/bulgarin/bulg-01.xml")
